...
def cleanse_freetrade(
    ft: pd.DataFrame, mic_remap: Dict, symbol_suffix: Dict
) -> pd.DataFrame:
    ...
    ft["stock_exchange"] = [
        mic_remap.get(x) for x in ft.mic
    ]  # convert mic to exchange e.g. XLON: London (INVESTPY)
    ft["symbol_alphavantage"] = ft.symbol.str.cat(
        [symbol_suffix.get(x, "") for x in ft.mic]
    )
...
def cleanse_investments(
    investments: pd.DataFrame, portfolio_remap: Dict
) -> pd.DataFrame:
...
def download_historic_alpha_vantage(
    stocks: pd.DataFrame, params: Dict[str, Any], access_key: str
...
def prepare_historic_alpha_vantage(
