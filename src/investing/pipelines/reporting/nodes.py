...
def combine_etf_outputs(
    ...
    merge2 = pd.merge(
        merge1, forecast, on="name", how="left", suffixes=("", "_forecast")
    )
    merge3 = pd.merge(merge2, etf_historical_meta, on="name", how="left")
    merge4 = pd.merge(
        merge3, current_holdings, on=["symbol_ft", "stock_exchange"], how="left"
    )
    return merge4
...
def enrich_etf_summary(
    clean_etf_summary: pd.DataFrame, buy_params: Dict
) -> pd.DataFrame:
    ...
    summary = (
        ...
        .assign(
            dividend_decimal=lambda df: df.dividend_yield.str.strip("%")
            .astype(float)
            .fillna(0)
            / 100
        )
...
def sell_etf(summary: pd.DataFrame, current_holdings: pd.DataFrame) -> pd.DataFrame:
    ...
    sell = pd.merge(
        summary, current_holdings, on=["symbol_ft", "stock_exchange"], how="right"
    )
...
def buy_etf(summary: pd.DataFrame, buy_params: Dict) -> pd.DataFrame:
    ...
    # rank output
    buy.sort_values(
        by=["expected_return", "pct_below_yhat_lower"], ascending=False, inplace=True
    )
